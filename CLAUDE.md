...
**Textos:** variables `{stat}` y `{meta.clave}`; género del protagonista `{o|a|x}` / `{héroe|heroína|herox}` (neutro con x) (`game.json → gender`; no en codex/items/traits/characters); rich text `[bold red]...[/bold red]`. El texto de la IA se sanea (`[` → `«`).
