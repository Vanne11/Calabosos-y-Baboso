...
export const processCommand = async (command, gameState, handlers = {}, sessionHandlers = {}) => {
  ...
  let commandLower = command.toLowerCase().trim();
  if (commandLower.startsWith('/')) {
    commandLower = commandLower.substring(1);
  }
...
[italic]Usa estos comandos si realmente quieres ver lo desastroso que es todo por dentro.[/italic]`;
        ...
        // Si es una promesa (como copy), esperarla
        if (debugOutput instanceof Promise) {
          debugOutput.then(result => {
            if (result && result.trim() !== '' && handlers.addToTerminal) {
            ...
            if (handlers.addToTerminal) {
              handlers.addToTerminal({
                type: 'system',
                content: `[red]Error en comando debug: ${error.message}[/red]`
              });
            }
          });
        } else {
          // Resultado sincrónico
...
[italic]¿Quizás sea una señal para hacer algo productivo con tu vida en lugar de jugar?[/italic]`;
    ...
    case 'exit':
      ...
      if (setLoggedUser) {
        setLoggedUser(null);
        if (isDebugActive()) {
          logDebug('Sesión cerrada correctamente', 'SYSTEM');
        }
      }
      ...
      if (addToTerminal) {
        addToTerminal({
          type: 'system',
          content: '[italic]Abandonando como siempre lo haces. Cerrando sesión...[/italic]'
        });
      }
