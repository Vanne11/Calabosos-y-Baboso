...
export const handleDebugCommand = (args, gameState, handlers) => {
  ...
  switch (subcommand) {
    ...
    case 'copy':
      ...
      // copyDebugToClipboard es async, pero handleDebugCommand no lo es
      // Retornamos una promesa que se resolverá
      return copyDebugToClipboard();

    case 'save':
      if (!isDebugActive()) {
        return '[yellow]Activa el modo depuración primero con "/debug on", cerebrito.[/yellow]';
      }
...
[green]/debug copy[/green] - Copia todo el log de depuración al portapapeles (para pegar en Discord y quedar como experto)
[green]/debug save[/green] - Descarga todo el log como archivo de texto (evidencia física de tu sufrimiento)
...
/**
 * Copia todo el historial de debug al portapapeles
 * @returns {Promise<string>} - Mensaje de confirmación
 */
export const copyDebugToClipboard = async () => {
  if (!isDebugActive()) {
    return '[yellow]El modo debug no está activo. No hay nada que copiar.[/yellow]';
  }

  try {
    // Generar el texto del log
    const logText = generateDebugLogText();

    // Copiar al portapapeles
    await navigator.clipboard.writeText(logText);

    return '[green]✓ Log de depuración copiado al portapapeles exitosamente.[/green]\n[dim]Ahora puedes pegarlo donde quieras y presumir tu debugging skills.[/dim]';
  } catch (error) {
    return `[red]Error al copiar al portapapeles: ${error.message}[/red]\n[dim]Intenta con /debug save en su lugar.[/dim]`;
  }
};

/**
 * Descarga el historial de debug como archivo de texto
 * @returns {string} - Mensaje de confirmación
 */
export const saveDebugToFile = () => {
  if (!isDebugActive()) {
    return '[yellow]El modo debug no está activo. No hay nada que guardar.[/yellow]';
  }
  ...
  try {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    link.download = `debug-log-${timestamp}.txt`;
    ...
    return '[green]✓ Log de depuración descargado exitosamente.[/green]\n[dim]Guárdalo como evidencia de tu persistencia (o masoquismo).[/dim]';
  } catch (error) {
    return `[red]Error al guardar el archivo: ${error.message}[/red]`;
  }
};

/**
 * Genera el texto completo del log de depuración
 * @returns {string} - Texto del log formateado
 */
const generateDebugLogText = () => {
  const lines = [];

  // Encabezado
  lines.push('═══════════════════════════════════════════════════════');
  lines.push('  MOTOR BABOSO - LOG DE DEPURACIÓN');
  lines.push('  "Porque hasta las babosas necesitan solucionar problemas"');
  lines.push('═══════════════════════════════════════════════════════');
  lines.push('');
  lines.push(`Fecha: ${new Date().toLocaleString()}`);
  lines.push(`Entradas totales: ${debugState.history.length}`);
  lines.push('');
  lines.push('───────────────────────────────────────────────────────');
  lines.push('');
  ...
  debugState.history.forEach((entry, index) => {
    ...
    lines.push(`[${index + 1}] ${timestamp} [${entry.category}] ${entry.message}`);
  });

  // Pie de página
  lines.push('');
  lines.push('───────────────────────────────────────────────────────');
  lines.push('Fin del log. Esperamos que esto te haya ayudado.');
  lines.push('(Aunque probablemente solo te confundió más)');
  lines.push('═══════════════════════════════════════════════════════');
